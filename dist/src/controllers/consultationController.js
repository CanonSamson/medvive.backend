...
export const initializeConsultation = asyncWrapper(async (req, res) => {
    ...
    try {
        ...
        if (!pendingConsultationsQuery.empty) {
            const pendingConsultations = pendingConsultationsQuery.docs.map((doc) => ({
                id: doc.id,
                ...doc.data()
            }));
            ...
            return res.status(409).json({
                error: 'Patient already has pending consultations. Please complete or cancel existing consultations before booking a new one.',
                success: false,
                pendingConsultations,
                pendingConsultationsCount: pendingConsultations.length
            });
    ...
    try {
        ...
        await createDBAdmin('consultation-transactions', transactionData?.transactionId, transactionData);
    ...
    if (result.success) {
        ...
        try {
            await sendEmail(email, `Consultation initiated with Dr. ${doctorData.fullName}`, 'default', {
                title: 'Consultation Initialized',
                text: `Hi ${firstName || 'Patient'}, your consultation with Dr. ${doctorData.fullName} has been initiated for ${date} at ${time}. Payment amount: ₦${Number(consultationFee.total)}. Virtual account: ${result?.data?.virtualBankAccountNumber || 'provided'}. Bank code: ${result?.data?.virtualBankCode || 'N/A'}. Transaction ID: ${result?.data?.transactionId || orderId}.`
            });
...
export const checkConsultationPaymentStatus = asyncWrapper(async (req, res) => {
    ...
    const transactionRecord = await getDBAdmin('consultation-transactions', transactionId);
    if (!transactionRecord.data) {
        logger.warn('Transaction status confirmation failed - transaction not found', {
            requestId,
            transactionId
        });
        return res.status(404).json({
    ...
    try {
        ...
        if (result.success) {
            ...
            try {
                await updateDBAdmin('consultation-transactions', transactionId, updateData);
            ...
            try {
                ...
                const isPaid = ['COMPLETED', 'SUCCESSFUL', 'SUCCESS', 'PAID'].includes(statusUpper);
                if (isPaid) {
                    const doctorRecord = await getDBAdmin('doctors', transactionData.doctorId);
                    const patientRecord = await getDBAdmin('patients', transactionData.patientId);
                    const doctorInfo = doctorRecord?.data;
                    const patientInfo = patientRecord?.data;
                    const doctorEmail = doctorInfo?.email || doctorInfo?.medviveEmail || '';
                    const patientEmail = patientInfo?.email || '';
                    // Notify doctor of a paid consultation
                    if (doctorEmail) {
                        await sendEmail(doctorEmail, `New paid consultation with ${patientInfo?.fullName || 'patient'}`, 'default', {
                            title: 'New Consultation Booked',
                            text: `A paid consultation with ${patientInfo?.fullName || 'a patient'} is scheduled for ${transactionData.consultationDetails?.date} at ${transactionData.consultationDetails?.time}. Consultation ID: ${transactionData.consultationId}.`
                        });
                    ...
                    if (patientEmail) {
                        await sendEmail(patientEmail, `Payment received - consultation confirmed with Dr. ${doctorInfo?.fullName || 'your doctor'}`, 'default', {
                            title: 'Consultation Confirmed',
                            text: `Hi ${patientInfo?.fullName?.split(' ')[0] || 'there'}, your payment has been received. Your consultation with Dr. ${doctorInfo?.fullName || ''} is confirmed for ${transactionData.consultationDetails?.date} at ${transactionData.consultationDetails?.time}. Consultation ID: ${transactionData.consultationId}.`
                        });
            ...
            try {
                await updateDBAdmin('consultation-transactions', transactionId, updateData);
                logger.info('Firebase transaction record updated with failed status check', {
                    requestId,
                    transactionId,
                    error: result.error
                });
            ...
            return res.status(400).json({
                ...
                error: 'Your transaction is being processed. If payment was completed, please wait a few minutes and check again. If no payment was made, please complete your payment to proceed.',
                requestId,
                timestamp: new Date().toISOString()
            });
            // }
