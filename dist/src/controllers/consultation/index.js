...
const convertToNigeriaTimezone = (date, format) => {
    try {
...
export const isPastSlotForDate = (selectedDate, slotMinutesFromMidnight, now) => {
...
const generateAvailableTimeSlots = (selectedDate, doctorAvailableSlots) => {
    ...
    const pastSlots = []; // Store past slots for visual feedback and accessibility
...
const generateDateOptions = (doctorAvailableSlots) => {
    const dates = [];
...
export const getDoctorTimeSlotsForDate = async (req, res) => {
    ...
    try {
        ...
        const { availableSlots: timeSlots, unavailableSlots } = generateAvailableTimeSlots(date, doctorAvailableSlots);
        logger.info('getDoctorTimeSlotsForDate: Successfully generated time slots', {
            requestId,
            doctorId,
            date,
            timeSlotsCount: timeSlots.length,
