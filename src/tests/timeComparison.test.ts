...
function run() {
  ...
  // Same day comparisons
  {
    ...
    assert.strictEqual(
      isPastSlotForDate(selectedDate, slot21, now),
      true,
      '21:00 should be past when now is 22:00 on same day'
    )

    assert.strictEqual(
      isPastSlotForDate(selectedDate, slot22_30, now),
      false,
      '22:30 should NOT be past when now is 22:00 on same day'
    )
  }
  ...
  // Different day: slots should not be considered past
  {
    ...
    assert.strictEqual(
      isPastSlotForDate(selectedDateNext, slot21, now),
      false,
      'Slots on a different day should not be considered past'
    )
  }
